...
Chọn theo thẻ (Tag): Chọn tất cả các thẻ cùng tên.Cú pháp: p, div, a, h1

Ví dụ: soup.select('a') (Lấy tất cả thẻ liên kết)

Chọn theo Class: Dùng dấu chấm . trước tên class.Cú pháp: .ten-class

Ví dụ: soup.select('.price') (Lấy phần tử có class="price")

Chọn theo ID: Dùng dấu thăng # trước tên ID.Cú pháp: #ten-id
...
Chọn con trực tiếp (Child Selector): Dùng dấu >. Chỉ chọn phần tử cấp con ngay sau nó.Cú pháp: div > p

Chọn tất cả con/cháu bên trong (Descendant Selector): Dùng khoảng trắng .Cú pháp: div p (Tìm tất cả thẻ p nằm trong div, bất kể sâu bao nhiêu)

Chọn phần tử anh em liền kề (Adjacent Sibling): Dùng dấu +.Cú pháp: h1 + p (Chọn thẻ p đầu tiên nằm ngay sau thẻ h1)
...
Mở trang web cần cào trên Chrome/Firefox.

Nhấn F12, chọn công cụ Inspect (Mũi tên) và click vào phần tử cần lấy.

Tại tab Elements, click chuột phải vào dòng code HTML đó → Copy → Copy selector.

Dán trực tiếp chuỗi vừa copy vào hàm .select() hoặc .css().
